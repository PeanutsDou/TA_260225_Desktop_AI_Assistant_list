...
# 邮件任务数据文件路径
# 注意：在服务端部署时，ai_konwledge 目录是 mock 的，可能不存在或者只读
# 我们优先尝试在当前目录下或者 ai_konwledge 下创建
if os.path.exists(os.path.join(project_root, "ai_konwledge")):
    EMAIL_TASKS_FILE = os.path.join(project_root, "ai_konwledge", "email_tasks.json")
else:
    # 兼容服务端环境，如果 ai_konwledge 不存在（虽然我们现在 mock 了），或者我们想用其他路径
    # 在服务端 server_dist 结构中，ai_konwledge 是存在的
    EMAIL_TASKS_FILE = os.path.join(project_root, "ai_konwledge", "email_tasks.json")
    
# 确保目录存在
os.makedirs(os.path.dirname(EMAIL_TASKS_FILE), exist_ok=True)
